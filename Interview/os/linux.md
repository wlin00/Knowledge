...
- **inode**：一个文件占用一个 inode，**每个inode都有一个独一无二的编号，在操作系统内部是以文件的inode编号进行操作的，与文件名无关，文件名主要用来寻找到inode编号，相当于域名和ip**，记录文件的属性，同时记录此文件的内容所在的 block 编号；
...
**目录也是文件**，建立一个目录时，会分配一个 inode 与至少一个 block。block 记录的内容是目录下所有文件的 inode 编号以及文件名。
...
**符号链接文件保存着源文件所在的绝对路径（有点类似间接索引）**，在读取时会定位到源文件上，可以理解为 Windows 的快捷方式。
...
| 9 | SIGKILL|立即杀死正在运行的进程，不能被阻塞，如果发现进程终止不了，可以尝试|
| 15 | SIGTEAM | 终止正在运行的进程，可以被阻塞，kill默认使用这个|
| 18 | SIGCONT | 让一个停止(stopped)的进程继续执行(处于R可执行态). 本信号不能被阻塞|
| 19| SIGSTOP | 让其为停止状态|
...
```sh
$ uptime
 20:33:28 up 10:53,  1 user,  load average: 1.49, 1.26, 1.37
```
也可以在top中查看
